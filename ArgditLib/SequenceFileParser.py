...
class SequenceFileParser:
    ...
    '''
    Function name: parse_ontology_schema_db
    ...
    '''
    def parse_ontology_schema_db(self, seq_file_path, is_nt_seq_only = False):
        self._init_data()
        ...
        with open(seq_file_path, 'rU') as f:
            for seq_record in SeqIO.parse(f, 'fasta'):
                ...
                if seq_type == '':
                    self._unknown_seq_type_seq_rec_ids.append(seq_record.id)
                    continue
                ...
                if acc_num_type == '' and seq_type != 'AA':
                    self._invalid_acc_num_fmt_seq_rec_ids.append(seq_record.id)
                    continue
        ...
        with open(seq_file_path, 'rU') as f:
            for seq_record in SeqIO.parse(f, 'fasta'):
                ...
                if Utils.is_duplicated_item(seq_record.id, validated_seq_headers):
                    self._duplicated_headers.append(seq_record.id)
                ...
                if seq_type == '':
                    ...
                    redundant_seq_id, is_rev_comp = Utils.check_redundant_seq(seq_record.seq, seq_record.id,
                                                                              validated_seq_strs, seq_type)
                    if redundant_seq_id is not None:
                        self._redundant_seq_pairs.append((seq_record.id, redundant_seq_id, is_rev_comp))
